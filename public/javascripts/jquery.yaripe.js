jQuery.fn.yaripe = function(url, objectName, attributeName) {
    ...
    function clickFunction() {
        ...
        case "date":
            ...
            break;
        default:
            var input = '<input id="inplace_input" class="email" type="text" value="' + oldValue + '">';
            break;
        }

        e.html('<form id="inplace" action="javascript:void(0)" style="display:inline;" onkeypress="cancelInPlaceEdit(event, this);">' + input + '<input name="submit" value="Salvar" type="submit"> </form>');
        ...
        e.find("form#inplace").submit(function(){
			...
			var value_email = e.find(".email").val();
	
			if(value == ""){
				alert("Tem que preencher o campo!");
				e.find("#inplace_input").focus();
			}else{
			if(!emailPadrao.test(value_email)){
					alert("Test");
					e.find(.email).focus();
				}
